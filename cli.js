  ...
  if (relayerURL) {
    ...
    try {
      ...
      if (netId === 56 || netId === 97) {
        console.log(`Transaction submitted through the relay. View transaction on BSC scan https://bscscan.com/tx/${relay.data.txHash}`)
      } else {
 ...
 */
async function init({ rpc, noteNetId, currency = 'bnb', amount = '0.1' }) {
  ...
  try {
    if (web3 == undefined) {
      ...
      contractJson = await (await fetch('build/contracts/BNBSmashnado.json')).json()
  ...
  window.deposit = async (_currencyStr, _amountStr) => {
    ...
    try {
      ...
      if (noteString) {
        ...
        resultStr += 'Sender account BNB balance is ' + web3.utils.fromWei(await web3.eth.getBalance(senderAccount)) +'<br/>'
